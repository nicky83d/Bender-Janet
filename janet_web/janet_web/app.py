...
def create_app(janet):
    ...
    def detection_info():
        data=janet.state.get()
        return jsonify({'status':'ok','model':data.get('detection_model',{}),'fps':data.get('fps',0),'detections':data.get('detections',[]),'candidates':config.DETECTION_MODEL_CANDIDATES})

    @app.route('/detection_scan', methods=['POST'])
    def detection_scan():
        results=config.DETECTION_MODEL_CANDIDATES
        ...
        return jsonify({'status':'ok','message':'Configured model candidates listed. Use one at a time and watch FPS.','results':results})

    @app.route('/detection_use_model', methods=['POST'])
    def detection_use_model():
        p=request.get_json(silent=True) or {}; ok,msg=janet.vision.set_model(p.get('model'))
        return jsonify({'status':'ok' if ok else 'error','message':msg})
