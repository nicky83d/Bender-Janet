...
const SKILL_VIEWS = ['boot','motors','routines','detection','oak-d','voice','speech','elevenlabs','faces','objects','hermes'];
...
function switchView(view){
  ...
  if(view==='boot') loadBoot(); if(view==='routines') loadRoutines(); if(view==='detection') loadDetectionInfo(); if(view==='faces') loadFaceInfo(); if(view==='objects') loadObjectInfo(); if(view==='elevenlabs') loadElevenLabs(); if(view==='oak-d') loadOakDExamples();
}
...
function bind(){
  ...
  $('mode-btn')?.addEventListener('click', (e) => {
    ...
    if(!menu.classList.contains('hidden')) loadOakDExamples();
  });
  ...
  $('ai-detection-btn')?.addEventListener('click', ()=> { aiDetectionEnabled = !aiDetectionEnabled; $('ai-detection-btn').classList.toggle('active', aiDetectionEnabled); postJson('/ai_detection_toggle', {enabled: aiDetectionEnabled}).then(d => setText('dashboard-status', d.message || 'AI detection toggled')); });
  $('oak-d-disable')?.addEventListener('click', ()=> { postJson('/oak_d_example', {example: 'bender-janet'}).then(d => { setText('oak-d-status', d.message || 'Returned to normal mode'); loadOakDExamples(); }); });
  ...
  if($('ai-detection-btn')) { $('ai-detection-btn').classList.add('active'); }
}
...
async function loadAIDetectionState(){ const d=await api('/readings_basic'); const det=d.detection||{}; aiDetectionEnabled = det.ai_enabled!==false; $('ai-detection-btn')?.classList.toggle('active', aiDetectionEnabled); }

function colorClass(v){ if(v<0 || v<20) return 'bad'; if(v<50) return 'warn'; return 'ok'; }
function estimateDistanceMeters(d){
  const xmin = Number(d?.xmin);
  const xmax = Number(d?.xmax);
  const ymin = Number(d?.ymin);
  const ymax = Number(d?.ymax);
  if([xmin,xmax,ymin,ymax].some(v => Number.isNaN(v))) return null;
  const w = Math.max(0.001, xmax - xmin);
  const h = Math.max(0.001, ymax - ymin);
  const area = Math.max(0.00001, w * h);
  const meters = Math.max(0.3, Math.min(12, 0.35 / Math.sqrt(area)));
  return meters;
}
function updateReadings(data){
  ...
  setHtml('detections-list', det.length ? `<div class="detections-grid">${det.map(d=>{
    const dist = estimateDistanceMeters(d);
    const distTxt = dist == null ? '?m' : `${dist.toFixed(1)}m`;
    return `<div class="det-tile"><div class="det-name">${esc(d.label)}</div><div class="det-meta">${distTxt} - ${esc(d.confidence)}%</div></div>`;
  }).join('')}</div>` : '<p class="note">No detections</p>');
...
if(document.readyState==='loading'){document.addEventListener('DOMContentLoaded',initializeDashboard);}else{initializeDashboard();}
